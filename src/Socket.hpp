...
class Socket {
  ...
  public:
    // Basic constructor for creating a socket with the specified hostname and
    // port
    ...
    std::pair<int, std::string> connect2Client() {
        ...
        std::pair<int, std::string> result = std::make_pair(
            client_connection_fd, getIP((struct sockaddr *)&socket_addr));
        return result;
    }
