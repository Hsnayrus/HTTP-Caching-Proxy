...
*/
    void writeRequest(size_t id, std::string request, std::string ipAddress) {
        std::string requestReceived = std::to_string(id) + ": " + request +
                                      " from " + ipAddress + "@" +
                                      getGMtime(time(0));
    ...
    void writeBeforeSend(size_t id, std::string request, std::string url) {
        std::string beforeSend = std::to_string(id) + ": " + "Requesting " +
                                 request + " from " + url + "\n";
        writeToFile(beforeSend);
    }

    void writeAfterReceive(size_t id, std::string response, std::string url) {
        std::string afterSend = std::to_string(id) + ": " + "Received " +
                                response + " from " + url + "\n";
    ...
    void writeinCacheExpired(size_t id, time_t inputTime) {
        std::string toWrite = std::to_string(id) +
    ...
    void writeNotCacheable(size_t id) {
        std::string toWrite =
            std::to_string(id) + ": not cacheable because no-store\n";
        writeToFile(toWrite);
    }
