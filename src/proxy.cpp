...
void use_get(Parser & newparser, int client_fd, Cache & cacheStorage,
    ...
    if ((cachedResponse != NULL) && (!cachedResponse->needRevalidation)) {
        ...
        send(client_fd, cachedResponse->content.data(),
             cachedResponse->content.size(), 0);
        ...
        try {
            newLog.writeBeforeSend(id, newparser.getFirstline(),
                                   newparser.getHostName());
        } catch (std::iostream::failure & e) {
            std::cerr << e.what();
            return;
        }
        ...
        while (len > 0) {
            ...
            if (len > 0) {
                newbuffer.insert(newbuffer.end(), buffer.begin(),
                                 buffer.begin() + len);
            }
        }
        ...
        try {
            newLog.writeAfterReceive(id, rParser.getFirstline(),
                                     newparser.getHostName());
        } catch (std::iostream::failure & e) {
            std::cerr << e.what();
            return;
            ...
            if (cachedResponse != NULL) {
                try {
                    newLog.writeinCacheExpired(
                        id, (cachedResponse->responseReceivedTime +
                             (int)cachedResponse->maxAge));
                } catch (std::iostream::failure & e) {
                    std::cerr << e.what();
                    return;
                }
                cacheStorage.updateValue(newparser.getUrl(), newResponse);
                std::cout << "Updated the response value for url: "
                          << newparser.getUrl() << std::endl;
            } else {
                try {
                ...
                std::cout << "Inserting new response for url: "
                          << newparser.getUrl() << std::endl;
            }
            try {
                newLog.writeCached(id, newResponse.responseReceivedTime +
                                           newResponse.maxAge);
            } catch (std::iostream::failure & e) {
                std::cerr << e.what();
                return;
...
int main() {
    ...
    while (true) {
        ...
        std::thread connection(&threadConnections, clientInfo.first,
                               &cacheStorage, ids, clientInfo.second);
        connection.detach();
        ids++;
    }
    return 0;
